...
public class QrProvider {
    ...
    public void drawDotImage(final int x, final int y, final int dotSize, final Graphics2D graphics, final String resourceImage) {
        try (InputStream inputStream = QrProvider.class.getResourceAsStream(resourceImage)) {
            ...
            AttendanceMonitoringLogger.errorMessage(this.getClass(), ex);
        }
    }

    private BufferedImage readImageResource(String resourceLogo) {
        try (InputStream inputStream = QrProvider.class.getResourceAsStream(resourceLogo)) {
            if (inputStream != null) {
                return ImageIO.read(inputStream);
            }
        } catch (NullPointerException | IOException ex) {
            AttendanceMonitoringLogger.severe(QrProvider.class.getName(), "No image '" + resourceLogo + "' found!");
        }
        return null;
    }
